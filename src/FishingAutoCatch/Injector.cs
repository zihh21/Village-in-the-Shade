...
{
    /// <summary>
    /// 注入器：向目标进程分配内存写入 stub，并打五个 16 进制 patch：
    ...
    ///   Hook 点 E：0x215D2B（state2 拉线/咬钩确认键 0x498 判定点，16 字节，resume 0x215D3B）
    ///     v1.0.4 新增：自动模式无条件写 [r14+0x1DC]=3（等效自动按 0x498，创建节奏任务实现全自动链）；
    ///     F8 关/背包满完整复放原版 test sil,sil（40 84 F6）+ je 0x215D4D / 写 3，语义与原版一致。
    ///
    /// 状态字节（stub 缓冲区 704 字节尾部[672,704)）：gEnabled(1B) + gHits(4B) + gBagFull(1B)
    /// + gFishCount(4B) + gEntryA/C/D/E（四口入口计数，各 4B）。
    /// </summary>
    internal static class Injector
    {
        ...
        public static string Install(IntPtr hProcess, long pid, long moduleBase)
        {
            ...
            long targetE = moduleBase + HookStub.HookERva;

            // 0) 防重复注入：若是本工具上次打的补丁，先还原为原始字节再重打
            RestoreIfDoubleInject(hProcess, pid, moduleBase, targetA, targetB, targetC, targetD, targetE);

            // 1) 读取五处被覆盖前的原始字节（用于状态文件/还原）
            ...
            var originalE = new byte[HookStub.HookEPatchLength];
            if (!ReadAt(hProcess, targetE, originalE))
                return "读取 Hook 点 E 原始字节失败（句柄权限不足？）。";
            ...
            if (!WriteAt(hProcess, stubBase, code))
                return "写入 stub 代码失败。";
            // gEnabled=1(默认开启), gHits=0, gBagFull=0, gFishCount=0, gEntryA/C/D/E=0
            ...
            // 4) 五个 Hook 点 detour / NOP
            //    Hook A → stub 区开头（Hook A 区偏移 0）；Hook C → stubBase+CastStubOffset（续竿区）；
            //    Hook D → stubBase+BagStubOffset（入包区）；Hook E → stubBase+HookEStubOffset（自动拉线）。
            //    若 C/D/E 也指向 stubBase 会跳进 Hook A stub，在 state5/state4/state2 上下文中乱读 rcx 并
            //    trampoline 到节奏判定内部，破坏整个钓鱼状态机（v1.0.0 缺陷）。
            ...
            if (!ProtectWriteVerify(hProcess, targetE,
                    BuildDetourPatch(stubBase + HookStub.HookEStubOffset, HookStub.HookEPatchLength)))
                return "写入 detour（Hook E）失败或校验不一致。";

            // 5) 记录状态并返回
            PatchState.Save(pid, moduleBase, stubBase, originalA, originalB, originalC, originalD, originalE);
            return $"注入成功。\n" +
                   $"  Hook A    : 0x{targetA:X}（入口 detour）\n" +
                   $"  Hook B    : 0x{targetB:X}（成功检查跳过）\n" +
                   $"  Hook C    : 0x{targetC:X}（续竿/自动重甩）\n" +
                   $"  Hook D    : 0x{targetD:X}（入包/背包满检测）\n" +
                   $"  Hook E    : 0x{targetE:X}（state2 自动拉线）\n" +
                   $"  Stub 基址 : 0x{stubBase:X}\n" +
                   $"  默认开关  : 开启（加载器按 F8 切换，开启 880Hz/关闭 440Hz 提示音）";
        }

        /// <summary>
        /// 若五个 Hook 点当前正是本模块上次写入的内容，则先还原原始字节并释放旧 stub，再让 Install 重打。
        ...
        private static void RestoreIfDoubleInject(IntPtr hProcess, long pid, long moduleBase,
            long targetA, long targetB, long targetC, long targetD, long targetE)
        {
            ...
            var curE = new byte[HookStub.HookEPatchLength];
            bool eOurs = originalE != null && ReadAt(hProcess, targetE, curE)
                         && IsOurPatch(curE, stubBase + HookStub.HookEStubOffset);
            if (!wasOurs && !bOurs && !cOurs && !dOurs && !eOurs) return;
            ...
            if (eOurs && originalE != null) ProtectWriteVerify(hProcess, targetE, originalE);
        ...
        public static string Remove(IntPtr hProcess, long pid, long moduleBaseNow)
        {
            ...
            // Hook E（v1.0.4）：detour 指向自动拉线 stub 区才还原
            if (originalE != null)
            {
                var curE = new byte[HookStub.HookEPatchLength];
                if (!ReadAt(hProcess, targetE, curE)) return "读取 Hook 点 E 失败。";
                if (!IsOurPatch(curE, stubBase + HookStub.HookEStubOffset))
                    return "Hook 点 E 当前内容不是本模块的 patch，跳过还原（可能已被其他工具修改）。";
                if (!ProtectWriteVerify(hProcess, targetE, originalE))
                    return "还原 Hook E 失败或校验不一致。";
            }

            Win32Api.VirtualFreeEx(hProcess, new IntPtr(stubBase), IntPtr.Zero, 0x8000 /*MEM_RELEASE*/);
            return "已还原五个 Hook 点的原始代码并释放 stub 内存。";
        }
        ...
        {
            if (!PatchState.TryLoad(pid, out _, out var stubBase, out _, out _, out _, out _, out _)) return -1;
            var buf = new byte[1];
        ...
        /// <summary>健康检查：五个 Hook 点是否在位（含 C/D/E 指向正确 stub 区）、Hook B 与开关状态是否一致、开关状态与四条计数。</summary>
        public static string Verify(IntPtr hProcess, long pid, long moduleBase)
        {
            if (!PatchState.TryLoad(pid, out var m, out var stubBase, out _, out var originalB, out _, out _, out _))
            ...
            // Hook C/D/E 的 detour 必须指向各自 stub 区（v1.0.0 曾误指向 stubBase 导致状态机错乱）
            long targetC = moduleBase + HookStub.CastRva;
            var curC = new byte[HookStub.CastPatchLength];
            if (!ReadAt(hProcess, targetC, curC)) return "读取 Hook 点 C 失败。";
            ...
            if (!ReadAt(hProcess, targetE, curE)) return "读取 Hook 点 E 失败。";
            if (!IsOurPatch(curE, stubBase + HookStub.HookEStubOffset))
                return "Hook 点 E detour 未指向自动拉线 stub 区（旧版本注入，缺 v1.0.4 Hook E），需重新注入。";
            ...
            int entryE = BitConverter.ToInt32(st, 22);
            ...
            return $"Hook A/B/C/D/E 均在位且与开关一致。开关={enabled}（1=开），" +
                   $"自动判定累计触发 {hits} 次，本次已钓 {fishCount} 条，背包满={bagFull}，" +
                   $"入口计数 A={entryA} C={entryC} D={entryD} E={entryE}（>0 表示游戏确实执行到对应 Hook 点）。";
        }
        ...
        {
            if (!PatchState.TryLoad(pid, out var m, out _, out _, out var originalB, out _, out _, out _))
        ...
        public static (int enabled, int hits, int bagFull, int fishCount,
                       int entryA, int entryC, int entryD, int entryE)? ReadLiveState(IntPtr hProcess, long pid, long moduleBase)
        {
            if (!PatchState.TryLoad(pid, out var m, out var stubBase, out _, out _, out _, out _, out _)) return null;
            ...
            return (st[0], BitConverter.ToInt32(st, 1), st[5], BitConverter.ToInt32(st, 6),
                    ...
                    BitConverter.ToInt32(st, 22));
        }

        /// <summary>读取五个 Hook 点当前字节（十六进制），用于注入/自检的关键字节比对日志。</summary>
        public static string DumpHookBytes(IntPtr hProcess, long moduleBase)
        {
            ...
            var e = new byte[HookStub.HookEPatchLength];
            if (!ReadAt(hProcess, targetE, e)) return "读取 Hook 点 E 失败。";

            return "A=" + Convert.ToHexString(a) + " | B=" + Convert.ToHexString(b) +
                   " | C=" + Convert.ToHexString(c) + " | D=" + Convert.ToHexString(d) +
                   " | E=" + Convert.ToHexString(e);
        }
