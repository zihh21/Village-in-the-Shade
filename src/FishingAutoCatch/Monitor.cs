...
namespace FishingAutoCatch
{
    /// <summary>
    /// 监控模式（默认入口，双击直接使用）：
    ///   1. 若游戏未启动 → 持续等待（每秒探测一次），启动后自动注入（Hook A + Hook B）；
    ///   2. 注入后保持轮询：开关变化、自动判定触发次数实时打印；
    ...
    internal static class Monitor
    {
            ...
            try
            {
                using (var p = ProcessManager.FindVillageProcess())
                {
                    ...
                    using (var handle = new SafeProcessHandle(p))
                    {
                        ...
                        string txt = newState == 1 ? "已开启自动判定" : "已关闭自动判定";
        ...
        /// <summary>约 500ms 一次：等待/注入游戏，或轮询开关与命中计数。</summary>
        private static void PollState(ref bool injected, ref long lastPid, ref int lastEnabled, ref int lastHits)
        {
            try
            {
                using (var p = ProcessManager.FindVillageProcess())
                {
                    ...
                    using (var handle = new SafeProcessHandle(p))
                    {
                        ...
                        {
                            string check = Injector.Verify(handle.Handle, moduleBase);
                            if (!check.StartsWith("Hook A/B 均在位"))
                            {
